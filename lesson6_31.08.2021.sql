...
SELECT SUM(file_size), media_types_id FROM media GROUP BY media_types_id;

-- считаем количество медиафайлов по каждому типу с названиями типов
SELECT COUNT(*) AS files_count, 
		(SELECT name FROM media_types WHERE id = media.media_types_id) as file_type
FROM media
...
SELECT * FROM friend_requests WHERE from_user_id = 1;

-- выбираем кому пользователь отправил заявки
SELECT to_user_id FROM friend_requests WHERE from_user_id = 1 AND request_type = 1;

-- выбираем от кого пользователю пришли заявки, заявки приня
...
SELECT 
	DISTINCT IF(to_user_id = 1, from_user_id, to_user_id) AS friend
FROM friend_requests
	WHERE (from_user_id = 1 OR to_user_id = 1) AND request_type = 1;
	
/*
 * Задание 6. Выводим имя и фамилию друзей пользователя с id = 1
*/

SELECT concat(first_name, ' ', last_name) AS name FROM users WHERE id IN (2,3,5,7,11); 

SELECT concat(first_name, ' ', last_name) AS name FROM users WHERE id IN (
	SELECT to_user_id FROM friend_requests WHERE from_user_id = 1 AND request_type = 1
	UNION
	SELECT from_user_id FROM friend_requests WHERE to_user_id = 1 AND request_type = 1
);
